...
void timeout_callback(void *arg)
{
    ...
    uart_send_string(")\r\n");
    // uart_send_string("(execute at %lu, current time %lu)\n",
    //                  data->message,
    //                  data->execute_time / (get_timer_freq()),
    //                  current_time / (get_timer_freq()));
}
...
void cmd_setTimeout(int argc, char **argv)
{
    ...
    // 創建 timeout 數據
    timeout_data_t *data = simple_alloc(sizeof(timeout_data_t));
...
void add_timer(void (*callback)(void *data), void *data, uint64_t duration)
{
    ...
    // 創建新計時器
    timer_t *new_timer = simple_alloc(sizeof(timer_t));
